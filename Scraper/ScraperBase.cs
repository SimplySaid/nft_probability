...
namespace Scraper
{
    public abstract class ScraperBase
    {
        ...
        public ScraperBase(IRpcClient client, ILogger logger, string name, string symbol, string address, string separator)
        {
            _client = client;
            _logger = logger;
            _name = name;
        ...
        {
            //Console.WriteLine(new MemCmp(69, Encoders.Base58.EncodeData(Encoding.UTF8.GetBytes(name)));
            //Console.WriteLine(Encoders.Base58.EncodeData(Encoding.UTF8.GetBytes(symbol)));
            var accounts = await _client.GetProgramAccountsAsync(MetaplexHelpers.ProgramId,
                memCmpList: new List<MemCmp>
                {
                    new () { 
        ...
        protected async Task<List<AccountKeyPair>> GetMetadataAccounts(string creator)
        {
            //Console.WriteLine(new MemCmp(69, Encoders.Base58.EncodeData(Encoding.UTF8.GetBytes(name)));
            var accounts = await _client.GetProgramAccountsAsync(MetaplexHelpers.ProgramId,
                memCmpList: new List<MemCmp>
                {
                    new () {
                        Offset = 69, //69
                        Bytes = Encoders.Base58.EncodeData(Encoding.UTF8.GetBytes(creator))
                    }
                });
            return accounts.Result;
        }

        protected async Task<MetadataAccountWrapper> GetArweaveData(MetadataAccountWrapper item, string creatorAddress) {
            ...
            if (arweaveData.Properties.Creators[0].Address != creatorAddress && creatorAddress != "")
            {
                Console.WriteLine(arweaveData.Properties.Creators[0].Address);
                return null;
            }
